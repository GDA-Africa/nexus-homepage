...
## [2026-03-07] bug-fix — Homepage animation: S4 missing closing tags breaks S5/S6/S7
The `<div id="s4" class="scene">` was never closed — S5, S6, and S7 were nested inside S4 as children. Because `.scene` is `position:absolute;opacity:0;pointer-events:none` and only `.scene.active` is visible, when `setScene(5)` added `.active` to `#s5` it simultaneously removed `.active` from `#s4`. Since `#s5` is a child of `#s4`, the parent's `opacity:0` made S5 invisible even though `#s5` had `.active`. Fix: added `</div></div>` to properly close S4's inner wrapper and scene div before the S5 comment. Pattern: every `<!-- Sn: TITLE -->` scene div must be self-contained; verify with HTML nesting before debugging JS.

## [2026-03-07] bug-fix — Homepage animation: stage click-guard logic inverted for pause state
The stage click listener must follow this exact precedence:
1. If main-site is faded in (opacity > 0.5) → return early (animation over)
2. If `_paused === true` → call `togglePause()` and return (any click unpauses)
3. If tag is button/input/a → return (let native click fire)
4. If `closest()` matches any interactive component selector → return
5. Otherwise → call `togglePause()` (pause on empty background click)

The selector list for `closest()` must include the **container panels** too, not just the cards: `.s5-type-card,.s6-tool-card,.s5-term-panel,.s6-snippet`. Without the panels, clicking inside the terminal panel (not on a card) still triggers pause. Never add `span` to the passthrough tag list — it would prevent pausing on empty stage background clicks which are often `<div>` or `<span>` wrappers.

## [2026-03-07] gotcha — Homepage animation: _ixAdvance must guard with !_interactiveMode
`_ixAdvance(nextFn, delay)` is called at the end of interactive-mode scene completions. It already guards with `if(!_interactiveMode)return` so it is safe to call from scene functions that run in both modes (e.g. `_unlockNode`). Do NOT remove this guard — without it, auto-mode would double-advance scenes because both the absolute T() timeline wipes AND the _ixAdvance wipes would fire for the same scene transition.
